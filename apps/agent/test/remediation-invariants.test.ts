...
describe('remediation invariants', () => {
  ...
  test('event reconciliation decodes text fields and matches Walrus blob bytes to original blob ids', async () => {
    ...
    expect(eventSource).toContain('Event reconciliation is the durable trigger');
    expect(eventSource).not.toContain("data: { verificationStatus: 'SCANNING' }");
    expect(eventSource).toContain("verificationStatus: 'UPLOADED'");
    expect(eventSource).toContain('enqueueSubmittedDeliverableVerification');
    expect(eventSource).toContain('optionalDateFromMs');
    expect(eventSource).toContain("txType: 'SUBMIT_DELIVERABLE'");
    ...
    expect(signerSource).toContain('Confirmation pending. TrustLine will keep watching this transaction.');
    ...
    expect(repairSource).toContain('payerApprovalDeadlinesCleared');
  ...
  test('factual relationship memory is persisted independently from semantic AI search', async () => {
    ...
    expect(memorySource).toContain('RELATIONSHIP_MEMORY_PERSIST_REQUESTED');
    expect(memorySource).toContain('persistRelationshipMemoryEntry');
    expect(outboxSource).toContain('RELATIONSHIP_MEMORY_PERSIST_EVENT');
    expect(outboxSource).toContain('previous batch is still running');
    expect(walrusSource).toContain('memwal.remember(');
    expect(walrusSource).not.toContain('rememberAndWait(');
    ...
    expect(managementSource).toContain('relationshipId: rel.id');
    expect(managementSource).toContain('tenantId: rel.tenantId');
